...
vector<int> db = {10,13,16,19,
                20,22,24,
                30,28,27,26,
                25,30,35,40};
vector<vector<int>> db_id = {
                        {0,1,2,3,11,12,13,14},
                        {4,5,6,11,12,13,14},
...
void go(int cnt, int score, vector<ii> cur_status){
    ...
    for(int i{}; i < 4; ++i){
        ...
        if(cur_status[i].first == 0){
            ...
            if(new_pos >= db_id[new_status.first-1].size()){//도착
                new_status = {5,0};
            }else{
        ...
        for(int j = 0; j < 4; ++j){
            ...
            if(cur_status[j].first && new_status.first){
                int d1 = db_id[cur_status[j].first-1][cur_status[j].second];
                int d2 = db_id[new_status.first-1][new_status.second];
                if(d1 == d2){
                    same = true;
                    break;
                }
        ...
        if(!same) {
            ...
            if(new_status.first == 0){
                go(cnt+1,score + new_status.second*2, next_status);
            }
            else if(new_status.first == 5)//이동한 말이 도착한 경우
                go(cnt+1,score,next_status);
            else
                go(cnt+1,score + db[db_id[new_status.first-1][new_status.second]],next_status);
        }
